...
def parse_option():
    ...
    parser.add_argument('--ratio', type=float, default=1.0,
                        help='ratio of training data used to train the attack model')
...
target_path = "./save/%s/model_%s_bs_%s_dataset_%s/" % (
    opt.method, opt.model, opt.batch_size, opt.dataset)


#! Note that we only train the simCLR for one target task, so for multible OL dataset like CelebA, we only store one model
if opt.dataset == "CelebA":
    aux_label_simclr = "Smiling"
elif opt.dataset == "UTKFace":
    aux_label_simclr = "Race"
elif opt.dataset in ["Place365", "Place100", "Place50", "Place20"]:
    aux_label_simclr = "category"

model_name = 'combined_model_%s_with_adv_factor_%s_advimage_%s_adv_location_%s_%s_%s.pth' \
    % (opt.mode, opt.adv_factor, opt.adv_image, opt.adv_location, opt.original_label, aux_label_simclr)
...
target_combine_model = _load_model(
    target_combine_model, target_path + model_name)
...
with open("log/model/exp_attack/ol_adv_simclr.txt", "a") as wf:
    wf.write("%s,%s,%s,%s,%s,%s,%d,%s,%s,%.3f,%.3f,%.3f,%.3f\n" % (opt.method, opt.dataset, opt.model, opt.adv_factor, opt.adv_image,
             opt.adv_location, epoch_train, opt.original_label, opt.aux_label, target_train_acc, target_test_acc, train_acc, test_acc))
